...
int main(int argc, char **argv) {
	...
	for (int itries = 0; itries < 1; itries++) {
		...
		if (dvrk_inverse.ik_solve(affine_gripper_wrt_base) > 0) {
			...
			if (refinement_outcome > 0) {
				...
				std::cout << "NO BETTER SOLUTION!" << std::endl;
			}
	...
	std::cout << "Without additional Jacobian correction, \e[31m\e[1m" << err_cnt << " \e[0mof them failed the error assessment*."
			<< std::endl;
	...
	std::cout << std::endl << " * error assessment: " << std::endl
			...
			<< std::endl << "   tip_err.norm() > 0.001" << std::endl;
