//
// Created by william on 06/11/18.
//
...
void gen_rand_joint_perturbations(davinci_kinematics::Vectorq7x1 &dqvec, double eps = 0.000001)
{
  ...
  // TODO delete
  std::cout << "dqvec: " << std::endl;
  std::cout << dqvec.transpose() << std::endl;

}


void gen_rand_single_joint_perturbations(davinci_kinematics::Vectorq7x1 &dqvec, int joint_number, double eps = 0.000001)
{
  static unsigned int seed = time(NULL);
  dqvec(6) = 0;
  ...
  // joint_number from 0 as the first joint.
  dqvec(joint_number) = eps * (1.0 - 0.5*static_cast<double> (rand_r(&seed)) / static_cast<double> (RAND_MAX));
...
{
  ros::init(argc, argv, "davinci_Jacobian_test_rn");
  ...
  davinci_fwd_solver.resetDhOffsetsMaps();
  davinci_fwd_solver.loadDHyamlfiles("dh_info_format","psm1_dh");
  ...
  for (int itries = 0; itries < tries_target; itries++) // used to be 10000
  {
    davinci_fwd_solver.gen_rand_legal_jnt_vals(q_vec1);

    std::cout << "using q_vec = " << q_vec1.transpose() << std::endl;

    std::vector<double> p_fractional_err;
    p_fractional_err.clear();
    std::vector<double> r_fractional_err;
    r_fractional_err.clear();




    affine_fk1 = davinci_fwd_solver.fwd_kin_solve(q_vec1, "psm1_dh");
//    affine_fk1 = davinci_fwd_solver.fwd_kin_solve(q_vec1);




    for (int jt_n = 0; jt_n < 5; jt_n ++) {
      ROS_WARN("Joint#%d", jt_n+1);

      gen_rand_single_joint_perturbations(dq_vec, jt_n);
      ...
      dq_vec6x1(0) = davinci_fwd_solver.j1_scale_factor_map_["psm1_dh"] * dq_vec6x1(0);
      dq_vec6x1(1) = davinci_fwd_solver.j2_scale_factor_map_["psm1_dh"] * dq_vec6x1(1);
      dq_vec6x1(2) = davinci_fwd_solver.j3_scale_factor_map_["psm1_dh"] * dq_vec6x1(2);
      ...
      affine_fk2 = davinci_fwd_solver.fwd_kin_solve(q_vec2, "psm1_dh");
//      affine_fk2 = davinci_fwd_solver.fwd_kin_solve(q_vec2);
      ...
      Jacobian = davinci_fwd_solver.compute_jacobian(q_vec1, "psm1_dh");
//      Jacobian = davinci_fwd_solver.compute_jacobian(q_vec1);
      ...
      std::cout << " = p_err/dp_fk.norm()" << std::endl;
      std::cout << " p_err: " << p_err << std::endl;
      std::cout << " dp_fk.norm(): " << dp_fk.norm() << std::endl;

      p_fractional_err.push_back(p_err / (dp_fk.norm()) );
      ...
      std::cout << " = phi_err/dphi_fk.norm()" << std::endl;
      std::cout << " phi_err: " << phi_err << std::endl;
      std::cout << " dphi_fk.norm(): " << dphi_fk.norm() << std::endl;

      r_fractional_err.push_back(phi_err / (dphi_fk.norm()) );
...
//    std::cout << "The B*inv(B):\n" << rn_test << std::endl;
    ...
    for (int n = 0; n < 5; n ++) {
      ...
      std::cout << "p_fractional_err Joint#" << n+1 << " :" << p_fractional_err[n] << std::endl;

    }
    std::cout <<  "------- " << std::endl;
    for (int n = 0; n < 5; n ++) {

      std::cout << "r_fractional_err Joint#" << n+1 << " :" << r_fractional_err[n] << std::endl;
