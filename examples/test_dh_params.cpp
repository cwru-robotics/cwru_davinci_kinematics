...
int main(int argc, char **argv)
{
  ...
  Eigen::Affine3d affine_gripper_wrt_base_1, affine_gripper_wrt_base;
  ...
  Rot << 1,0,0,
      0,1,0,
      0,0,1;
  affine_gripper_wrt_base_1.linear() =Rot;
  affine_gripper_wrt_base_1.translation() << 0,0,0;

  if (dvrk_inverse.ik_solve(affine_gripper_wrt_base_1) > 0) {

    q_vec_ik = dvrk_inverse.get_soln();
    std::cout << "q_vec_ik#=============INPUT=BY=USER=============================" << ": " << std::endl;
    std::cout << q_vec_ik.transpose() << std::endl << std::endl;
