...
namespace davinci_kinematics {
	...
	class Forward {
		public:
			...
			Eigen::Affine3d fwd_kin_solve(const Vectorq7x1 &q_vec, std::string kinematic_set_name, const unsigned int desired_joint = 7);
...
// TODO make private later, it is put here to debug
		...
		private:
			...
			void fwd_kin_solve_DH(const Eigen::VectorXd &theta_vec, const Eigen::VectorXd &d_vec, std::string kinematic_set_name);
